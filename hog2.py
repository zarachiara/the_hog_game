...
def roll_dice(num_rolls, dice=six_sided_dice, who='Boss Hogg'):
    ...
    "*** YOUR CODE HERE ***"
    summing = 0 # track the number of points
    counting = 0 # counter 
    while num_rolls > counting: 
        result = dice() # dice() takes in the result
        if commentary:
            announce(result, who)
        if result == 1: 
            return 1 # is dice rolled was one, just return 1 
        else: 
            summing = result + summing # add up the number of roles from dice()
        counting += 1 # increment add by one
    return summing # return total number of points

# Write your own prime functions here!
"***YOUR CODE HERE***"
def is_prime(x): # identifies if number is prime 
    k = 2 
    if x == 1: 
        return False
    while k < x: 
...
def take_turn(num_rolls, opponent_score, dice=six_sided_dice, who='Boss Hogg'):
    ...
    turnScore = 0
    if num_rolls > 0: 
        turnScore = roll_dice(num_rolls, dice, who)
    else:
