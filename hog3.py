...
def full_bacon(opponent_score):
    """Returns opponent score when hogitmus prime, touchdown, or freebacon are present.  These cases will be used in the final strategy. See final strategy function for more details"""
...
def make_mean_strategy(min_points, num_rolls=5):
    ...
    def mean_strategy(score, opponent_score):
        ...
        if bacon_points < min_points: 
            return num_rolls
        if hog_tied(bacon_points, opponent_score) or hog_wild(bacon_points, opponent_score): 
            return 0
        else: 
            return num_rolls
...
def final_strategy(score, opponent_score):
    ...
    The strategies below emphasize when we should take advantage of obtaining free bacon points when rolling zero. 

    The most important rule is determined when all the special cases apply to opponent score and when score is greater than goal, we must return 0. 
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if hog_tied(bacon_points, opponent_score) and bacon_points > 5:
        return 0
    if hog_wild(bacon_points, opponent_score) and bacon_points > 5:
        return 0

    if bacon_points >= 11 and diff < 11:
        return 0
